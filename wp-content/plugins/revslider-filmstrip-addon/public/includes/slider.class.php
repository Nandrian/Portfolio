...
class RsFilmstripSliderFront extends RevSliderFunctions {
	...
	public function add_addon_files($html, $export){
		...
		if(!$export->usepcl){
			$export->zip->addFile(RS_FILMSTRIP_PLUGIN_PATH . 'public/assets/js/revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js', 'js/revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js');
			$export->zip->addFile(RS_FILMSTRIP_PLUGIN_PATH . 'public/assets/css/revolution.addon.' . $this->pluginTitle . '.css', 'css/revolution.addon.' . $this->pluginTitle . '.css');
		}else{
			$export->pclzip->add(RS_FILMSTRIP_PLUGIN_PATH.'public/assets/js/revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js', PCLZIP_OPT_REMOVE_PATH, RS_FILMSTRIP_PLUGIN_PATH.'public/assets/js/', PCLZIP_OPT_ADD_PATH, 'js/');
			$export->pclzip->add(RS_FILMSTRIP_PLUGIN_PATH.'public/assets/css/revolution.addon.' . $this->pluginTitle . '.css', PCLZIP_OPT_REMOVE_PATH, RS_FILMSTRIP_PLUGIN_PATH.'public/assets/css/', PCLZIP_OPT_ADD_PATH, 'css/');
		}
	...
	private function isEnabled($slider){
		...
		foreach($slides as $slide){
			if(
				$this->get_val($slide, array('params', 'addOns', 'revslider-filmstrip-addon', 'enable'), false) === true &&
				$this->get_val($slide, array('params', 'addOns', 'revslider-filmstrip-addon', 'settings', 0, 'url'), '') !== ''
			){
				$enabled = true;
				break;
			}
	...
	public function add_html_script_additions($return, $output){
		...
		$return['toload']['filmstrip'] = '<script'. $addition .' src="'. $this->pluginUrl . 'public/assets/js/revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js"></script>';
	...
	public function write_init_script($slider, $id){
		...
		if(!empty($enabled)){
			...
			$settings = $this->get_val($slider, 'settings', array());
			$type = $this->get_val($settings, 'type', 'standard');
		
			$title    = $this->pluginTitle;
			$base     = $this->pluginUrl;
			$carousel = $type !== 'carousel' ? 'false' : 'true';
			$id       = $slider->get_id();
