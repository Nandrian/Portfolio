...
class RevAddOnDuotoneUpdate {
	...
	protected function _check_updates($force_check = false){
		...
		$last_check = get_option('revslider_duotone_addon-update-check');

		if($last_check == false){ //first time called
			$last_check = time();
			update_option('revslider_duotone_addon-update-check', $last_check);
		}
		...
		if(time() - $last_check > 60 * 60 * 24 * 30 || $force_check == true){
			...
			if(isset($data->basic)){
				update_option('revslider_duotone_addon-update-check', time());
				
				$this->data->checked = time();
				$this->data->basic = $data->basic;
				$this->data->full = $data->full;
				
				update_option('revslider_duotone_addon-latest-version', $data->full->version);
			}
		}
