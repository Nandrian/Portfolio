...
class RsRefreshSliderFront extends RevSliderFunctions {
	...
	public function add_addon_files($html, $export){
		...
		$_jsPathMin = file_exists(RS_REFRESH_PLUGIN_PATH . 'public/assets/js/revolution.addon.' . $this->pluginTitle . '.js') ? '' : '.min';
		if(!$export->usepcl){
			$export->zip->addFile(RS_REFRESH_PLUGIN_PATH . 'public/assets/js/revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js', 'js/revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js');
		}else{
			$export->pclzip->add(RS_REFRESH_PLUGIN_PATH.'public/assets/js/revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js', PCLZIP_OPT_REMOVE_PATH, RS_REFRESH_PLUGIN_PATH.'public/assets/js/', PCLZIP_OPT_ADD_PATH, 'js/');
		}
	...
	public function add_html_script_additions($return, $output){
		...
		$return['toload']['refresh'] = '<script'. $addition .' src="'. $this->pluginUrl . 'public/assets/js/revolution.addon.' . $this->pluginTitle . $_jsPathMin . '.js"></script>';
	...
	public function write_init_script($slider, $id) {
		...
		$id = $slider->get_id();
		echo "if(typeof RsRefreshAddOn !== 'undefined') {RsRefreshAddOn(revapi" . $id . ", '" . json_encode($addOn) . "');}". "\n";
