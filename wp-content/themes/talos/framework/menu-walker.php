 ...
 * @since 1.0.0
 */
class talos_Walker_Mega_Menu extends Walker_Nav_Menu {
	/**
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( 1 == $depth && $this->in_mega ) {
			$classes[] = 'mega-sub-menu col-'. esc_attr( $item_column );
		}
		...
		if ( 1 <= $depth && ! empty( $item_content ) ) {
			$item_output .= '<span class="new-menu">' .do_shortcode( $item_content ) . '</span>';
		}

		/** Add custom badge */
		// if ( in_array( 'hot', $classes )  ) {
		// 	$item_output .= '<span class="hot-menu">' . esc_html__( 'Hot', 'talos' ) . '</span>';
		// } elseif ( in_array( 'new', $classes ) ) {
		// 	$item_output .= '<span class="new-menu">' . esc_html__( 'New', 'talos' ) . '</span>';
		// }
