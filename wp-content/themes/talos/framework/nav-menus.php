 ...
 * @since 1.0
 */
class talos_Walker_Nav_Menu_Custom_Fields {
	/**
	...
	public function __construct() {
		...
		$this->fields = array(
			'mega_menu'       => esc_html__( 'Mega Menu', 'talos' ),
			'mega_menu_width' => esc_html__( 'Mega Menu Width', 'talos' ),
			'column'          => esc_html__( 'Column Width', 'talos' ),
			'content'         => esc_html__( 'Content', 'talos' ),
			'hide_text'       => esc_html__( 'Hide Text', 'talos' ),
			'bg_image'        => esc_html__( 'Background Image', 'talos' ),
			'bg_h_position'   => esc_html__( 'Horizontal Position', 'talos' ),
			'bg_v_position'   => esc_html__( 'Vertical Position', 'talos' ),
			'bg_repeat'       => esc_html__( 'Repeat', 'talos' ),
			'bg_size'         => esc_html__( 'Size', 'talos' ),
		);

		add_action( 'wp_ajax_talos_mega_menu', array( $this, 'talos_save_mega_menu' ) );
		add_action( 'wp_ajax_nopriv_talos_mega_menu', array( $this, 'talos_save_mega_menu' ) );

		add_action( 'wp_ajax_talos_mega_fields', array( $this, 'talos_mega_fields' ) );
		add_action( 'wp_ajax_nopriv_talos__mega_fields', array( $this, 'talos_mega_fields' ) );
	}
	...
	public function enqueue_scripts( $hook ) {
		...
		wp_enqueue_style( 'talos-nav-menus', get_template_directory_uri() . '/css/nav-menus.css', array(), false );
		wp_enqueue_script( 'talos-nav-menus', get_template_directory_uri() . '/js/nav-menus.js', array(), false );
		wp_localize_script( 'talos-nav-menus', 'talos', array(
			'nonce'    => wp_create_nonce( '_talos_nonce' ),
			'megamenu' => esc_html__( 'Mega Menu', 'talos' ),
			'savemenu' => esc_html__( 'Save Menu', 'talos' )
		) );
	}
	 ...
	 *
	 */
	public function talos_save_mega_menu() {
		check_ajax_referer( '_talos_nonce', 'lpnonce' );
	 ...
	 */
	public function talos_mega_fields() {
		check_ajax_referer( '_talos_nonce', 'lpnonce' );
		...
		<div class="field-mega-menu-bg_image field-mega-menu field-of-mega-menu">
			...
			<div class="background-preview <?php echo esc_attr( $class_image ); ?>" id="background-preview"
				...
				<button type="button" id="upload-image"
						class="btn-select"><?php esc_html_e( 'Upload Image', 'talos' ); ?></button>
				<input type="hidden" id="edit-item-mega-menu-bg_image" value="<?php echo esc_url( $bg_image ); ?>">
				<button type="button" id="remove-image"
						class="btn-select btn-remove"><?php esc_html_e( 'Remove Image', 'talos' ); ?></button>
			</div>
		</div>
		...
		<div class="field-mega-menu-bg_content field-mega-menu field-of-mega-menu">
			<p>
				...
				<select id="edit-item-mega-menu-bg_h_position">
					<option
						value="left" <?php selected( 'left', $bg_h_position ); ?>><?php esc_html_e( 'Left', 'talos' ) ?></option>
					<option
						value="right" <?php selected( 'right', $bg_h_position ); ?>><?php esc_html_e( 'Right', 'talos' ) ?></option>
					<option
						value="center" <?php selected( 'center', $bg_h_position ); ?>><?php esc_html_e( 'Center', 'talos' ) ?></option>
				</select>
			</p>
			...
			<p>
				...
				<select id="edit-item-mega-menu-bg_v_position">
					<option
						value="top" <?php selected( 'top', $bg_v_position ); ?>><?php esc_html_e( 'Top', 'talos' ) ?></option>
					<option
						value="bottom" <?php selected( 'bottom', $bg_v_position ); ?>><?php esc_html_e( 'Bottom', 'talos' ) ?></option>
					<option
						value="center" <?php selected( 'center', $bg_v_position ); ?>><?php esc_html_e( 'Center', 'talos' ) ?></option>
				</select>
			</p>
			...
			<p>
				...
				<select id="edit-item-mega-menu-bg_repeat">
					<option
						value="no-repeat" <?php selected( 'no-repeat', $bg_repeat ); ?>><?php esc_html_e( 'No Repeat', 'talos' ) ?></option>
					<option
						value="repeat" <?php selected( 'repeat', $bg_repeat ); ?>><?php esc_html_e( 'Repeat', 'talos' ) ?></option>
					<option
						value="repeat-x" <?php selected( 'repeat-x', $bg_repeat ); ?>><?php esc_html_e( 'Repeat Horizontally', 'talos' ) ?></option>
					<option
						value="repeat-y" <?php selected( 'repeat-y', $bg_repeat ); ?>><?php esc_html_e( 'Repeat Vertically', 'talos' ) ?></option>
				</select>
			</p>
			<p>
				...
				<select id="edit-item-mega-menu-bg_size">
					<option
						value="" <?php selected( '', $bg_size ); ?>><?php esc_html_e( 'Normal', 'talos' ) ?></option>
					<option
						value="contain" <?php selected( 'contain', $bg_size ); ?>><?php esc_html_e( 'Contain', 'talos' ) ?></option>
					<option
						value="cover" <?php selected( 'cover', $bg_size ); ?>><?php esc_html_e( 'Cover', 'talos' ) ?></option>
				</select>
			</p>
		...
		<div class="field-mega-content field-mega-menu field-not-mega-menu">
			...
			<span class="description"><?php esc_html_e( 'HTML and Shortcodes are allowed', 'talos' ); ?></span>
		</div>
		<?php
		$output = ob_get_clean();
	...
	public function menu_walker( $walker ) {
		$walker = 'talos_Walker_Nav_Menu_Edit';
 ...
 * @since 1.0
 */
class talos_Walker_Nav_Menu_Edit extends Walker_Nav_Menu_Edit {
	/**
